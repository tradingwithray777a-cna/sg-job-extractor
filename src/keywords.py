...
def _ks_get(ks: Any, key: str, default: Any):
    """
    Supports:
      - dataclass/object: ks.key
      - dict: ks["key"]
    """
    if ks is None:
        return default
    if isinstance(ks, dict):
        return ks.get(key, default)
    return getattr(ks, key, default)
...
def run_search(
    ...
    ks_target = _ks_get(ks, "target_role", target_role) or target_role
    ks_core = _ks_get(ks, "core_keywords", []) or []
    ks_adj = _ks_get(ks, "adjacent_titles", []) or []
    ks_near = _ks_get(ks, "nearby_titles", []) or []
    ks_excl = _ks_get(ks, "exclude_keywords", []) or []

    queries = build_queries(target_role, ks_adj, ks_core)
    ...
    filtered = []
    excl = [_norm(x) for x in (ks_excl or [])]
    for r in raw_rows:
    ...
    # Deduplicate (title + employer) keep best completeness
    ...
    for r in deduped:
        r["Relevance score"] = compute_relevance(r, ks_target, ks_adj, ks_near)
    ...
    notes = {
        ...
        "TARGET_ROLE": ks_target,
        "Core keywords used": ", ".join([str(x) for x in ks_core]),
        "Adjacent titles used": ", ".join([str(x) for x in ks_adj[:10]]),
        "Exclude keywords used": ", ".join([str(x) for x in ks_excl]),
