...
def build_keyword_sets(target_role: str) -> KeywordSets:
    """
    Build keyword sets from TARGET_ROLE.
    Returns an object with attributes used by runner.py:
    - target_role, core_keywords, adjacent_titles, nearby_titles, exclude_keywords
    """
    tr = (target_role or "").strip()
    tr_n = _norm(tr)

    # Base synonyms (safe/general + a few role patterns)
    synonyms = {
    ...
    if "procurement" in tr_n:
        ...
        # Generic fallbacks for any other target role
        adjacent = [
            f"{tr} Executive",
            f"{tr} Specialist",
            f"Senior {tr}",
